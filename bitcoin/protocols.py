...
class SubsidyCalculator(object):
    def __init__(self, initial_payout, height, offset=0):
        self.__initial_payout = initial_payout
...
class ProtocolInfo(object):
    ...
    def __init__(self,
        ...
        self.__genesis_hash = genesis_hash
        self.__message_start = message_start
        self.__alert_key = alert_key
        self.__default_net_port = default_net_port
        self.__default_rpc_port = default_rpc_port
        self.__max_target = max_target
        self.__subsidy_calc = subsidy_calc
        self.__target_function = target_function
        self.__fixed_seeds = fixed_seeds
        self.__network_seeds = network_seeds
    ...
    def get_genesis_hash(self):
        return self.__genesis_hash

    def get_message_start(self):
        return self.__message_start

    def get_alert_key(self):
        return self.__alert_key

    def get_default_net_port(self):
        return self.__default_net_port

    def get_default_rpc_port(self):
        return self.__default_rpc_port

    def get_max_target(self):
        return self.__max_target

    def get_subsidy(self, height):
        return self.__subsidy_calc.get_subsidy(height)

    def get_target(self, targets, timestamps, height):
        return self.__target_function(self, targets, timestamps, height)

    def get_network_seeds(self):
        return self.__network_seeds

    def get_fixed_seeds(self):
        if not self.__fixed_seeds:
            return self.get_network_seeds()
        else:
            return self.__fixed_seeds

    def get_max_message_size(self):
        return self.__max_message_size

def convert_fixed_seeds(fixed_seeds):
    converted = []
    for i in fixed_seeds:
        converted.append("%d.%d.%d.%d" % ((i % 256), (i >> 8) % 256, (i >> 16) % 256, (i >> 24) % 256))
    return converted

MAIN_NET_FIXED_SEEDS = \
    [
...
MAIN_NET_INFO = ProtocolInfo("bitcoin_main",
...
TEST_NET_INFO = ProtocolInfo("bitcoin_test3",
